...
dfb <- dfbetas(fit2A)
# important dfbetas
idfb <- dfb[ , c("wt"  , "am", "wt:am")]
idfb <- as.data.frame(idfb)
idfb$model <- rownames(idfb)
colnames(idfb) <- c("wt",    "am",    "wt_am", "model")
...
mtcars2C <- filter(mtcars2B, model != "Toyota Corolla")
...
gdfb <- ggplot(idfb, aes(model)) +
        geom_point(aes(y = am, color= "am")) +
        geom_point(aes(y = wt_am, color= "wt:am")) +
        scale_color_discrete("coefficient") +
        scale_y_continuous("dfbeta") +
        theme(axis.text.x = element_text(angle=90, hjust=1))
...
pr <- par("mfrow")
par(mfrow=c(2,3))
for (i in 1:6) {plot(fit2B, which = i)}
par(mfrow=pr)
