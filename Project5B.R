...
cb <- influencegraph(fit2A, labnumber = 7, ylim1 = c(0,.65), ylim2 = c(0,.27), title = "Initial situation")
displaygraph(cb)
...
displaygraph(grid.arrange(grobs=list(cb, cbB, cbC),  ncol = 1))
...
# Example QUick-R (http://www.statmethods.net/advstats/bootstrapping.html)
...
# initial result
bs(formula = mpg ~  qsec + wt*am, data=mtcars2E, indices= 1:nrow(mtcars2E))
...
results <- boot(data=mtcars2E, statistic=bs,
                R=4000, formula = mpg ~  qsec + wt*am)

# view results
results

cnames<- c("am", "wt_am", "wt_0")
dfres <- as.data.frame(results$t) # bootstrap dataframe of results
colnames(dfres) <- cnames
origres <- results$t0 # original result
names(origres) <- cnames

lim <- quantile(dfres[["wt_am"]], probs =c(0.015, 0.985), na.rm = TRUE)

idx <- which(cnames == "wt_am")

conflevel <- .95
cfint <- boot.ci(results, type="bca", index= idx, conf = conflevel)

# confidence limits
cflim <- cfint$bca[4:5]

vlines <- data.frame(x = c(cflim[1],  origres["wt_am"], c(cflim[2])),
                     lt = factor(c(2,1,2)) )
...
ggplot(dfres, aes(wt_am)) +
        ...
        scale_x_continuous(paste("bootstrap sample","wt_am"), limits= lim) +
...
# plot(results, index=1) # am
# plot(results, index=2) # wt:am
# plot(results, index=3) # wt0

# get 95% confidence intervals
boot.ci(results, type="bca", index=1) # am
boot.ci(results, type="bca", index=2) # wt:am
bci3 <- boot.ci(results, type="bca", index=3) # wt0
...
#' # generate samples
#'
#' lsamp <- lapply(1:B, function(i) {sample(1:n, n, replace = TRUE)}) # list of B samples of size n
#'
