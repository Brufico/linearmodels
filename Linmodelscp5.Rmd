...
# title
title <- textGrob("Model: mpg ~ wt + am + qsec (without interaction)") # modified
...
grall <- arrangeGrob(grobs=list(gr1,gr2), ncol = 1, left = "residuals")
...
**Answer**: Based on a linear model of the fuel efficiency (`mpg`) of the cars in the dataframe 'mtcars', it appears that, in order to get a higher mpg, a manual gearbox is better than an automatic one for the lighter cars, and that the converse is true for the heavier cars. The neutral weight is about `r round(nwt,0) * 1000` pounds, a car with a manual gearbox being about `r - round(deltaslope, 1)` miles per gallon better than the same car fitted with an automatic gearbox, for a car weighing 1000 pounds less than the neutral weight. 

For a car heavier than the neutral weight, the reverse is true, an automatic gearbox providing an improvement of about `r - round(deltaslope, 1)` miles per gallon over a manual gearbox, for each increase of 1000 pounds. 
...
Thus, we eliminate one by one `r remlist`, and the resulting model formula is `r firstformula`, with an adjusted R² of `r sf1$adj.r.squared`.  The variable of interest `am` is part of this model.
...
* **Diagnostic 2: Outliers removal**: The model formula is `mpg ~ am*wt + qsec`. However, checking for influential outliers, we can see (plot _(@diagmodel2)_) some cars seem to alter substantially the results. we choose to remove some points, in order to reduce this influence. We wish to remove the points with the highest Cook's distance, but more importantly those with high  $dfbeta_{am}$  and $dfbeta_{am:wt}$, as $\beta_{am}$ and $\beta_{wt:am}$ are critical here (see below). We thus remove one by one `r allremovals`.
The diagnostics plots after all removals (plot _(@diagmodel3)_) are satisfactory.
...
* For $wt_0 = -\frac{\beta_{am}}{\beta_{am:wt}}$, we cannot, (as $\hat \beta_{am}$ and $\hat \beta_{am:wt}$ are not independent) and we must resort to a Bootstrap procedure (with `r bootstriter` simulated samples), and we get (see graph *(@bootstrap)*): point estimation: **`r nwt`** , 95% confidence interval `r Intw0`
...
If the car weight is 1000 pounds less than $wt_0$, the expected mpg of a car with a manual gearbox is better than that of the same car with a manual gearbox by $-\beta_{am:wt} \approx$ `r - round(deltaslope, 1)`. The reverse is true if the car is 1000 pounds heavier than $wt_0$ .
...
* (@diagmodel1) Linear model with and without interaction, residuals plots

```{r app_residuals, fig.height=sfigheight -.4}
displaygraph(grall)
```

* (@diagmodel2) Diagnostics plot for checking and removing influential points.

```{r app_removepoint,fig.height=vlfigheight + 2.5 , fig.width=vlfigwidth}
displaygraph(allremgraph)
```

* (@diagmodel3) Final check. The basic regression assumptions seem to hold

```{r app_finalcheck, fig.height=vsfigheight, fig.width=lfigwidth}
pr <- par("mfrow")
par(mfrow=c(1,3))
for (i in 1:3) {plot(fit2E, which = i)}
par(mfrow=pr)
